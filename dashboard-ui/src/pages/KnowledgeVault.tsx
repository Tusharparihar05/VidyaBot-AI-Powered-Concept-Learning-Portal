...
export default function KnowledgeVault() {
  ...
  return (
    ...
    >
      ...
      {!selectedTag && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{subjectGroups.length}</span>
          </div>
            ...
            {subjectGroups.map(({ tag, count }, i) => {
              ...
              return (
                <motion.div
                  ...
                  onClick={() => setSelectedTag(tag)}
                  className={`${colors.bg} ${colors.border} border rounded-3xl p-5 cursor-pointer hover:-translate-y-1 transition-transform`}
                >
                  <div className={`${colors.folder} rounded-2xl p-3 ${colors.icon} w-fit mb-3`}>
                    {icon}
                  </div>
                  <h3 className="text-sm font-bold text-gray-800 capitalize">{tag.replace('_', ' ')}</h3>
                  <p className="text-xs text-gray-500 mt-1">{count} question{count !== 1 ? 's' : ''}</p>
                </motion.div>
              );
            })}
          </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <h3 className="text-sm font-bold text-gray-800">{selectedTag ? `${selectedTag.replace('_', ' ')} Questions` : 'Recent Questions'}</h3>
          <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{filteredHistory.length}</span>
        </div>

        {filteredHistory.length === 0 ? (
          <p className="text-xs text-gray-400 py-8 text-center">No questions found. Start learning on the Dashboard!</p>
        ) : (
              ...
              return (
                ...
                >
                  <div className={`w-9 h-9 rounded-xl ${colors.bg} ${colors.border} border flex items-center justify-center shrink-0`}>
                    <BookOpen size={15} className={colors.icon} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800 truncate">{item.rawQuestion}</p>
                    <p className="text-xs text-gray-400 truncate mt-0.5">
                      {item.textAnswer ? item.textAnswer.slice(0, 80) + '...' : 'No answer'}
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-[10px] font-medium bg-gray-100 text-gray-600 px-2 py-1 rounded-full capitalize">{item.subjectTag}</span>
      ...
      {chats.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <h3 className="text-sm font-bold text-gray-800">Active Chats</h3>
            <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{chats.length}</span>
          </div>
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm divide-y divide-gray-50">
            {chats.slice(0, 10).map((chat, i) => (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                transition={{ delay: i * 0.04 }}
                className="flex items-center gap-3 p-4 hover:bg-gray-50 transition-colors first:rounded-t-3xl last:rounded-b-3xl"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">{chat.messageCount} messages · {chat.subjectTag}</p>
                </div>
